...
var blockArray = [ //代表第一种可能出现的方块组合： Z
...
{
    var rand = Math.floor(Math.random() * blockArray.length);
    currentFall = [
        { x:blockArray[rand][0].x, y:blockArray[rand][0].y,color: blockArray[rand][0].color},
        { x:blockArray[rand][1].x, y:blockArray[rand][1].y,color: blockArray[rand][1].color},
        { x:blockArray[rand][2].x, y:blockArray[rand][2].y,color: blockArray[rand][2].color},
        { x:blockArray[rand][3].x, y:blockArray[rand][3].y,color: blockArray[rand][3].color}
    ...
    else
    {
        ...
        for (var i = 0; i < currentFall.length;i++)
        {
            ...
            if (cur.y < 2)
            {
                ...
                localStorage.removeItem("tetris.status");
...
{ //依次遍历每一行
    ...
    if(flag)
    {
        ...
        if(curScore >= curSpeed * curSpeed *  500)
        {
            ...
            clearInterval(curTimer);
            curTimer = setInterval("moveDown();", 500 / curSpeed); // 这行略有不懂
        }
        //把当前行的所有方块往下移动一行
        for (var k = i; k > 0; k--)
        {
            for (var l = 0; l < TETRIS_COLS; i ++)
            {
...
window.onload = function()
{
    ...
    document.body.appendChild(tetris_canvas);
     ...
     //读取local storage里的teris_status记录
